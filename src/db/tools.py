...
def get_unload_setting(is_local: bool=False):
    """
    返回字典形式：
        reload port 和 truck 类型（LL， LA， AL ，AA） 的映射
    examples:
        {'r1_1': ['LL'], 'r3_1': ['LL', 'LA']}
    """
...
def get_pipeline(is_local: bool=False):
    ...
    table = table[['equipment_port_last', 'equipment_port_next', 'sorter_zone']]
    table['machine_name'] = table['sorter_zone'].apply(lambda x: x[:2]).replace(machine_dict)
    return table


def get_queue_io(is_local: bool):
    """返回 io 对: [(r1_1,  m1_1), (r1_3, m2_3), ]"""
    table = get_pipeline(is_local)
    io_list = []
    for _, row in table.iterrows():
